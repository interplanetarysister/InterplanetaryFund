...
export default function FraudControl({ sessionId }: { sessionId: string }) {
  ...
  const handleApprove = async (payoutId: string) => {
    resetMessage();
    try { await approvePayout({ sessionId: sessionId as any, payoutId: payoutId as any }); setSuccess("Payout approved for completion."); }
    catch (e) { fail(e); }
  };
  ...
  const handleDeny = async (payoutId: string) => {
    ...
    resetMessage();
    try {
      await denyPayoutMutation({ sessionId: sessionId as any, payoutId: payoutId as any, reason: denyReason.trim() });
  ...
  const handleFreeze = async () => {
    ...
    resetMessage();
    try {
      await freezeCampaign({ sessionId: sessionId as any, campaignId: campaignId.trim() as any, reason: freezeReason.trim() });
  ...
  const handleUnfreeze = async (id: string) => {
    resetMessage();
    try { await unfreezeCampaign({ sessionId: sessionId as any, campaignId: id as any }); setSuccess("Campaign unfrozen."); }
    catch (e) { fail(e); }
  };

  const handleVerifyProof = async (id: string) => {
    resetMessage();
    try { await verifyOwnership({ sessionId: sessionId as any, campaignId: id as any }); setSuccess("Ownership verified."); }
    catch (e) { fail(e); }
  };

  const handleRejectProof = async (id: string, reason: string) => {
    resetMessage();
    try { await rejectOwnership({ sessionId: sessionId as any, campaignId: id as any, reason }); setSuccess("Ownership rejected. Campaign frozen."); }
    catch (e) { fail(e); }
  };

  const handleRequestProof = async (id: string) => {
    resetMessage();
    try { await requestProof({ sessionId: sessionId as any, campaignId: id as any }); setSuccess("Ownership proof requested."); }
    catch (e) { fail(e); }
  };
