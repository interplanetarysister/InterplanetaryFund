...
export default function UserManagement({ sessionToken }: { sessionToken: string }) {
  ...
  const userDetails = useQuery(
    ...
    selectedUser ? { sessionToken: sessionToken, userId: selectedUser } : "skip"
  );
  ...
  const handleToggleAi = async (userId: string, current: boolean) => {
    setError(""); setSuccess("");
    try {
      await toggleAi({ sessionToken: sessionToken, userId, enabled: !current });
  ...
  const handleToggleStandard = async (userId: string, current: boolean) => {
    setError(""); setSuccess("");
    try {
      await toggleStandard({ sessionToken: sessionToken, userId, enabled: !current });
  ...
  const handleRequestAccess = async (userId: string) => {
    setError(""); setSuccess("");
    try {
      await requestAccess({ sessionToken: sessionToken, userId, message: accessMessage || undefined });
  ...
  const handleRevoke = async (userId: string) => {
    setError(""); setSuccess("");
    try {
      await revokeAccess({ sessionToken: sessionToken, userId });
  ...
  const handleUnlink = async (platformId: string) => {
    ...
    setError(""); setSuccess("");
    try {
      await unlinkPlatform({ sessionToken: sessionToken, platformId: platformId as any });
