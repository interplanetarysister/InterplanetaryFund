...
export const provisionAutomationLaneLock = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    if (records.length > 1) throw new Error("automation_lock_not_unique");
    if (records.length === 1) {
      return { success: true, created: false, state: "already_initialized", recordCount: 1 };
    }
    ...
    const provisioned = await ctx.db
      ...
      .withIndex("byName", (q) => q.eq("name", AUTOMATION_LOCK_KEY))
      .collect();
    if (provisioned.length !== 1) throw new Error("automation_lock_provisioning_failed");

    return { success: true, created: true, state: "initialized", recordCount: 1 };
  },
});
