...
export const toggleAgentAutomation = mutation({
  args: { sessionId: v.id("adminSettings"), agentName: v.string(), enabled: v.boolean() },
  handler: async (ctx, { sessionId, agentName, enabled }) => {
    const principal = await requireAdminSession(ctx, sessionId, "settings");
...
// =====================================================
// GET AUTOMATION STATUS — Read-only dashboard display
// =====================================================
...
export const runAtlasAutomation = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    if (stuckJoins.length > 0) tasks.push(`${stuckJoins.length} group joins pending >48h — may need manual approval`);
...
export const runDonorRelationsAutomation = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    if (totalCompleted > 0) tasks.push(`${totalCompleted} completed campaigns — generate thank-you sequences`);
...
export const runCoordinatorAutomation = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const a of allAgents) {
      ...
      if (!a.automationEnabled) tasks.push(`${a.name} automation is disabled`);
    }
    ...
    for (const p of pendingPosts) platformGroups[p.platform] = (platformGroups[p.platform] || 0) + 1;
